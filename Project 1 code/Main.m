...
%% Uncomment this data to run for the ieee test (14 bus) system
...
%% Functions
function[Zbetween,Zself1,Zself2]=TrImp(typeofTr,Bus1,Bus2,X,Xn)
if typeofTr==31
    Zbetween=inf;Zself1=inf;Zself2=inf;
elseif typeofTr==32
    Zbetween=inf;Zself1=inf;Zself2=X;
elseif typeofTr==33 
    Zbetween=0;Zself1=0;Zself2=0;

elseif typeofTr==34
    Zbetween=inf;Zself1=X;Zself2=inf;
elseif typeofTr==35 
    Zbetween=inf;Zself1=inf;Zself2=inf;
else
    Zbetween=X;Zself1=inf;Zself2=inf;
end
end

%%
function[F_Current]=Fault_Analysis(F_Type_B,V_F,F_bus,Bus,Z0n,Z1n,Z2n,ZF)
  numOnes = sum(F_Type_B);
      if numOnes == 4
        disp('3 phase fault');
        I0=0;I2=0;I1=V_F/(Z1n);
            elseif numOnes == 3
                disp('LLG');
                I1=V_F/(Z0n+Z1n+Z2n)
                a=Z2n*(Z0n+3*ZF);b=(Z0n+3*ZF+Z2n);
                I1=V_F/(Z1n+(a/b))
                I2=-I1*((Z0n+3*ZF)/(Z0n+3*ZF+Z2n))
                I0=-I1*((Z2n)/(Z0n+3*ZF+Z2n));
                
            elseif numOnes == 2 
                 if F_Type_B(4)==1
                    disp('SLG');
                    I1=V_F/(Z0n+Z1n+Z2n+ZF);I0=I1;I2=I0;
                 else
                     disp('LL');
                    I1=V_F/(Z1n+Z2n+ZF);I2=-I1;I0=0; 
                 end
